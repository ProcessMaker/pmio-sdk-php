...
class Oauth
{
    ...
    public function findOauthClientsWithHttpInfo($user_id, $page = null, $per_page = null)
    {
        ...
        if (!is_null($page) && ($page < 1.0)) {
            throw new \InvalidArgumentException('invalid value for "$page" when calling Oauth.findOauthClients, must be bigger than or equal to 1.0.');
        }

        if (!is_null($per_page) && ($per_page > 100.0)) {
            throw new \InvalidArgumentException('invalid value for "$per_page" when calling Oauth.findOauthClients, must be smaller than or equal to 100.0.');
        }
        if (!is_null($per_page) && ($per_page < 1.0)) {
            throw new \InvalidArgumentException('invalid value for "$per_page" when calling Oauth.findOauthClients, must be bigger than or equal to 1.0.');
        }
